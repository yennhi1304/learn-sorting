const API_URL = "https://https://learn-sorting.onrender.com/chat";
...
async function sendMessage() {
  ...
  // If the chatbot is sleeping 
  async function sendMessageToChatbot(text) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 8000);
    // 8s timeout = if sleeping, wake takes 20–60s so we show message early
    ...
    try {
      ...
      if (!res.ok) {
        // Server responding but still waking up
        showSleepMessage();
        return null;
      }
      ...
      // Errors caused by sleeping server or network timeout
      showSleepMessage();
      return null;
    }
  }
  ...
  function showSleepMessage() {
    const box = document.getElementById("chatbox"); // whatever element shows chat messages
    const msg = document.createElement("div");
    msg.className = "bot-message";
    msg.innerText = "Tony is sleeping 😴 Please wait 1 minute for him to wake up.";
    box.appendChild(msg);
  }
